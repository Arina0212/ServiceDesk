...
class DirectEmailChecker:
    ...
    def check_emails_once(self):
        ...
        try:
            print(
                f"[{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] Проверка почты...")
            ...
            if not GEMAIL or not PASSWORD:
                print(
                    f"[{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] Ошибка: Не настроены EMAIL_HOST_USER или EMAIL_HOST_PASSWORD в .env")
                return
            ...
            if not email_ids:
                print(
                    f"[{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] Новых писем нет")
                mail.close()
                mail.logout()
                return

            print(
                f"[{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] Найдено {len(email_ids)} новых писем")
            ...
            for e_id in email_ids:
                try:
                    ...
                    # Обрабатываем письмо
                    print(
                        f"[{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] Обрабатываем письмо: {subject}")
                    ...
                    print(
                        f"[{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] Письмо обработано, обращение #{ticket.id}")

                except Exception as e:
                    print(
                        f"[{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] Ошибка обработки письма: {e}")
                    continue
            ...
            print(
                f"[{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] Проверка завершена. Обработано писем: {processed_count}")
