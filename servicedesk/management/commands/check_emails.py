    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            if not GEMAIL or not PASSWORD:
                self.stdout.write(
                    self.style.ERROR("Не настроены EMAIL_HOST_USER или EMAIL_HOST_PASSWORD в .env")
                )
                return
            ...
            for e_id in email_ids:
                try:
                    ...
                    if verbose:
                        self.stdout.write(f"Письмо обработано, обращение #{ticket.id}")
            ...
            self.stdout.write(
                self.style.SUCCESS(f"Проверка почты завершена. Обработано писем: {processed_count}")
            )
